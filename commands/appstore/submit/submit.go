// Package submit implements `gplay appstore submit --store-package <sp>
...
func (p Payload) renderMarkdown(w io.Writer) error {
	...
	if _, err := fmt.Fprintf(w, "## appstore submit\n\n"); err != nil {
		return err
	}
...
// NewCommand returns the cobra command for `gplay appstore submit`.
...
comes from a prior upload: ` + "`gplay appstore apk upload`" + ` returns the apk ids,
` + "`gplay appstore image upload`" + ` the icon/screenshot ids, and
` + "`gplay appstore policy upload`" + ` the document ids cited by a policy
...
requires.`,
  ...
  gplay appstore submit --file hosted-app.json --store-package com.example.store --dry-run

  # Submit to Google review (immediate and irrevocable)
  gplay appstore submit --file hosted-app.json --store-package com.example.store --confirm`,
		Args:          cobra.NoArgs,
		SilenceUsage:  true,
		SilenceErrors: true,
