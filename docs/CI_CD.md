...
| `ci.yml`: **Docs sanity** | PR + push to `main` | verb gate (ADR-0019), em dash gate, shellcheck of the install scripts, the install script's fail-closed checksum test (offline), the gplay.sh Worker's install-route test (`node --test`, offline: serves the latest release tag, never `main`), required files. **Required check.** | none |
