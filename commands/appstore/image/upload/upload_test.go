...
import (
	...
	imagecmd "github.com/PollyGlot/google-play-cli/commands/appstore/image/upload"
...
func TestRun_rendersTrackingID(t *testing.T) {
	...
	for name, render := range map[string]func(io.Writer) error{
		...
		for _, want := range []string{"img-42", "com.example.store", "com.example.app", "appstore submit", "appIconId", "screenshotId[]"} {
...
func TestNewCommand_helpDocumentsTheVerb(t *testing.T) {
	...
	if cmd.Use != "upload <file>" {
		t.Errorf("Use = %q, want upload <file>", cmd.Use)
	}
	if cmd.Short == "" {
	...
	// for, that nothing is shown until `appstore submit`, the size/type limit,
