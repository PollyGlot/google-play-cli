...
import (
	...
	apkcmd "github.com/PollyGlot/google-play-cli/commands/appstore/apk/upload"
...
func TestRun_rendersTrackingID(t *testing.T) {
	...
	for name, render := range map[string]func(io.Writer) error{
		...
		for _, want := range []string{"apk-42", "com.example.store", "com.example.app", "appstore submit", "baseApkId"} {
...
func TestNewCommand_helpDocumentsTheVerb(t *testing.T) {
	...
	if cmd.Use != "upload <file.apk>" {
		t.Errorf("Use = %q, want upload <file.apk>", cmd.Use)
	}
	if cmd.Short == "" {
	...
	// for, that nothing ships until `appstore submit`, the size/type limit, and
