...
import (
	...
	policycmd "github.com/PollyGlot/google-play-cli/commands/appstore/policy/upload"
...
func TestRun_rendersTrackingID(t *testing.T) {
	...
	for name, render := range map[string]func(io.Writer) error{
		...
		for _, want := range []string{"file-42", "com.example.store", "com.example.app", "appstore submit", "documentResponse", appstore.DeclarationFileTypeDocument} {
...
func TestNewCommand_helpDocumentsTheVerb(t *testing.T) {
	...
	// for, that nothing is declared until `appstore submit`, the size limit, the
