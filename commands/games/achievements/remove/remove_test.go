...
import (
	...
	removecmd "github.com/PollyGlot/google-play-cli/commands/games/achievements/remove"
...
func TestRun_missingConfirm_exit3(t *testing.T) {
	...
	_, err := removecmd.Run(rc, removecmd.Input{ID: "a7"})
	var sfe interface {
		ExitCode() int
		Error() string
	}
...
func TestRun_dryRun_reportsGateAndDoesNotCall(t *testing.T) {
	rc := gamescmdtest.NewRC(t, noCall(t))
	r, err := removecmd.Run(rc, removecmd.Input{ID: "a7", DryRun: true})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRun_happyPath_deletesAndConfirms(t *testing.T) {
	...
	rc.Stderr = &stderr
	r, err := removecmd.Run(rc, removecmd.Input{ID: "a7", Confirm: true})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
