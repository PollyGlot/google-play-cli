...
import (
	...
	publishstatuscmd "github.com/PollyGlot/google-play-cli/commands/appstore/publishstatus/set"
...
// `appstore submit`, and the change is reversible in both directions.
func TestNewCommand_helpDocumentsTheVerb(t *testing.T) {
	...
	if !strings.HasPrefix(cmd.Use, "set ") {
		t.Errorf("Use = %q, want it to start with set", cmd.Use)
	}
	if cmd.Short == "" {
