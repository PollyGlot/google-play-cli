// Package set implements `gplay games achievements set <achievementId>`:
...
//
// set is a full PUT replace: the body sent REPLACES the resource's draft, so
// for a partial edit fetch the current config (`view --output json`), edit it,
// and resend it with --file. The field flags send only what they name.
package set
...
func Run(rc *kernel.RunContext, in Input) (output.Renderable, error) {
	id, err := gamescmd.RequireID(in.ID, "missing achievement id: gplay games achievements set <achievementId>")
	if err != nil {
		return nil, err
	}
	...
	if in.DryRun {
		return gamescmd.AchievementWritePayload{Verb: "set achievement", Target: target, DryRun: true, Body: body, Cols: cols}, nil
	}
	...
	return gamescmd.AchievementWritePayload{Verb: "set achievement", Target: target, Row: gamescmd.BuildAchievementRow(a), Cols: cols, Raw: raw}, nil
}

// NewCommand returns the cobra command for `gplay games achievements set`.
func NewCommand(boot kernel.Boot) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "set <achievementId>",
		Short: "Set an achievement configuration (draft, full replace)",
		Long: `Set (replace) an achievement configuration by its ID. The write affects the editable
...
edit it, and resend it with --file. The field flags (--name, --description,
--type, --initial-state, --point-value, --steps-to-unlock; --locale, default
en-US) send only what they name; --file supplies a full body verbatim and
...
shows the request body). GPLAY_READONLY refuses the live write (exit 4).`,
  ...
  gplay games achievements set CgkI4ZqN0pMUEAIQAQ --file achievement.json --dry-run
  gplay games achievements set CgkI4ZqN0pMUEAIQAQ --file achievement.json`,
