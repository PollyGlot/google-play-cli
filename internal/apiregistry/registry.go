...
var entries = []Entry{
	...
	{MethodID: "androidpublisher.edits.listings.patch", Commands: []string{"metadata apply"}, Note: "writes the changed fields of a locale already live (ADR-0011: missing is not empty)"},
	{MethodID: "androidpublisher.edits.listings.update", Commands: []string{"metadata apply"}, Note: "creates a locale absent online, which patch cannot (#561)"},
	...
	{MethodID: "androidpublisher.monetization.onetimeproducts.purchaseOptions.batchUpdateStates", Commands: []string{"iap apply"}, Note: "declarative `state:` on a purchase option; the only state write path of that sub-resource"},
	{MethodID: "androidpublisher.monetization.onetimeproducts.purchaseOptions.offers.activate", Commands: []string{"iap apply"}, Note: "declarative `state:` on a one-time offer"},
	...
	{MethodID: "androidpublisher.monetization.onetimeproducts.purchaseOptions.offers.cancel", Commands: []string{"iap apply"}, Note: "--confirm-gated: cancelling a pre-order offer is irreversible"},
	{MethodID: "androidpublisher.monetization.onetimeproducts.purchaseOptions.offers.deactivate", Commands: []string{"iap apply"}},
