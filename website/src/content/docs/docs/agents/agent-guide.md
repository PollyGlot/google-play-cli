...
gplay is designed to be **driven by AI agents**: coding assistants, CI bots,
autonomous release managers. This page is the contract an agent can rely on.

## Machine-stable output

- Piped output (and `CI=true`) defaults to **JSON**, so an agent running
  gplay through a shell tool gets JSON without asking.
