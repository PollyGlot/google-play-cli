...
### Release
Cutting a new published version of the **gplay CLI** itself: the versioned binaries, archives, checksums, SBOMs, cosign signature, Homebrew formula and GitHub Release. A Release is triggered by merging the rolling "release PR" maintained by release-please, which creates the `vX.Y.Z` tag; GoReleaser then builds and publishes the artifacts. A Release concerns the CLI distribution only — the *installer* needs no redeploy when a new version ships (`install.sh` resolves the latest release itself). The gplay.sh Worker is nonetheless redeployed on a published Release, to rebuild the binary-generated CLI reference on the docs site (see Deploy).

### Deploy
Publishing a new version of the **gplay.sh Worker** — the single Cloudflare Worker that serves both the website (landing + docs, as static assets) and the install script behind `https://gplay.sh/install`. A Deploy is `wrangler deploy` of `deploy/gplay.sh/` (preceded by a site build), triggered when the Worker's own code changes (`worker.js` / `wrangler.toml`), when the site changes (`website/**`), or on a published Release (to refresh the generated CLI reference) — but **not** by a push that only touches `install.sh` (the push trigger is path-filtered, and the Worker proxies `install.sh` live from `main`; a published Release deploys regardless of which files changed). Distinct from Release: a Deploy concerns the gplay.sh site + install proxy, a Release concerns the CLI artifacts. See [ADR-0025](./docs/adr/0025-website-served-from-install-worker.md).
