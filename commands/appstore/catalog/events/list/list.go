// Package list implements `gplay appstore catalog events list --since <t>
// --until <t>`: one page of update events for the apps eligible for an app
...
func Run(rc *kernel.RunContext, in Input) (output.Renderable, error) {
	start, end, err := appstorecmd.ValidateTimeRange(in.Since, in.Until)
	if err != nil {
		return nil, err
	}
	if in.PageSize < 0 {
	...
	if resp.NextPageToken != "" {
		rc.Notef("more update events available, re-run with --page-token %s (keeping the same --since/--until/--page-size: the API rejects a page token when any other parameter changes) for the next page.", resp.NextPageToken)
	}
...
--since and --until are BOTH REQUIRED by the API and are validated
...
exclusive, so an --until at or before --since is a usage error
...
--since/--until AND --page-size across pages (the API rejects a page
