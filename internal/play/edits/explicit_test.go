...
// noRequestClient fails the test on ANY HTTP request. The explicit-mode
...
func noRequestClient(t *testing.T) *http.Client {
	...
	return &http.Client{Transport: testkit.RoundTripFunc(func(req *http.Request) (*http.Response, error) {
		t.Errorf("explicit mode made an HTTP call it should not: %s %s", req.Method, req.URL)
		return nil, errors.New("unexpected request")
	})}
}

func TestWithEdit_explicitMode_reusesPinAndMakesNoLifecycleCalls(t *testing.T) {
	hc := noRequestClient(t)
...
func TestWithEdit_explicitMode_failureDoesNotAutoDiscard(t *testing.T) {
	...
	// edits.delete) so the user can retry or `gplay edits discard`. noRequestClient
...
func TestOpenExplicit_insertsOnceNoCommitNoDiscard(t *testing.T) {
	rt := &editsRT{t: t, editID: "edit-77"}
	id, err := edits.OpenExplicit(context.Background(), rt.client(), "com.example.app")
	if err != nil {
...
func TestOpenExplicit_editAlreadyExists_isConflict(t *testing.T) {
	...
	_, err := edits.OpenExplicit(context.Background(), rt.client(), "com.example.app")
...
func TestCommitExplicit_postsCommitOnly(t *testing.T) {
	rt := &editsRT{t: t, editID: "edit-9"}
	if err := edits.CommitExplicit(context.Background(), rt.client(), "com.example.app", "edit-9", edits.CommitOptions{}); err != nil {
...
func TestDiscardExplicit_deletesOnly(t *testing.T) {
	rt := &editsRT{t: t, editID: "edit-3"}
	if err := edits.DiscardExplicit(context.Background(), rt.client(), "com.example.app", "edit-3"); err != nil {
