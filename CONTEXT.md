...
_Avoid_: "IAP" for subscriptions (sibling catalogs, distinct resources); writing, deleting or "syncing" the legacy surface — it is read-only in gplay, and the only exit is the one-way `--migrate` promotion to v2 (slice #372).
