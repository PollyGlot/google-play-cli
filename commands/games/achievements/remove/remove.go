// Package remove implements `gplay games achievements remove <achievementId>`:
...
func Run(rc *kernel.RunContext, in Input) (output.Renderable, error) {
	id, err := gamescmd.RequireID(in.ID, "missing achievement id: gplay games achievements remove <achievementId>")
	if err != nil {
		return nil, err
	}
	if in.DryRun {
...
// NewCommand returns the cobra command for `gplay games achievements remove`.
func NewCommand(boot kernel.Boot) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "remove <achievementId>",
		Short: "Remove an achievement configuration (irreversible delete)",
		Long: `Remove (delete) an achievement configuration by its ID. This is irreversible.
...
first with --dry-run. GPLAY_READONLY refuses the live delete (exit 4).`,
		Example: `  gplay games achievements remove CgkI4ZqN0pMUEAIQAQ --dry-run
  gplay games achievements remove CgkI4ZqN0pMUEAIQAQ --confirm`,
