...
import (
	...
	imagecmd "github.com/PollyGlot/google-play-cli/commands/appstore/upload/image"
...
const (
	...
	defaultResp = `{"imageId":"img-42"}`
)

// pngHeader is the 8-byte PNG signature http.DetectContentType keys on. The
// command announces the type it sniffs from these bytes, never the extension.
...
// writeImage drops a real PNG-headered asset at an EXTENSION-LESS temp path:
// the endpoint accepts image/* only, and the type has to come from the bytes,
// so a screenshot an operator named `icon` must still upload correctly.
func writeImage(t *testing.T) string {
	t.Helper()
	p := filepath.Join(t.TempDir(), "icon")
	if err := os.WriteFile(p, append(append([]byte{}, pngHeader...), make([]byte, 64)...), 0o600); err != nil {
...
func TestRun_requestShape(t *testing.T) {
	...
	r, err := imagecmd.Run(rc, imagecmd.Input{StorePackage: "com.example.store", Package: "com.example.app", Path: writeImage(t)})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
	if !strings.Contains(rt.initURL, "/appstore/com.example.store/apps/com.example.app/images:upload") {
		t.Errorf("url %q is not the appstoreappsreview.uploadImage endpoint", rt.initURL)
	}
	...
	if !bytes.HasPrefix(rt.putBody, pngHeader) {
...
// TestRun_sniffsContentTypeFromBytes asserts the initiate announces the type
// read from the file's leading bytes, not application/octet-stream: this
// endpoint declares image/* and would reject a blanket binary type only after
// the whole transfer had been paid for. The file is named `icon` with no
// suffix, so the extension cannot be the source of the answer.
func TestRun_sniffsContentTypeFromBytes(t *testing.T) {
	rt := &testRoundTripper{}
	rc := newRC(t, rt)

	if _, err := imagecmd.Run(rc, imagecmd.Input{StorePackage: "com.example.store", Package: "com.example.app", Path: writeImage(t)}); err != nil {
		t.Fatalf("Run: %v", err)
	}
	if rt.initCT != "image/png" {
		t.Errorf("X-Upload-Content-Type = %q, want image/png sniffed from the PNG signature", rt.initCT)
	}
}
...
func TestRun_rendersTrackingID(t *testing.T) {
	...
	for name, render := range map[string]func(io.Writer) error{
		...
		for _, want := range []string{"img-42", "com.example.store", "com.example.app", "appstore update", "appIconId", "screenshotId[]"} {
...
func TestRun_jsonPassthrough(t *testing.T) {
	const body = `{"imageId":"img-42","unmodeledFutureField":"kept"}`
...
func TestPayload_jsonEmptyBodyFallback(t *testing.T) {
	p := imagecmd.Payload{StorePackage: "com.example.store", Package: "com.example.app", ImageID: "img-42"}
	var out bytes.Buffer
...
func TestRun_packageDefaultsToProjectPin(t *testing.T) {
	...
	if _, err := imagecmd.Run(rc, imagecmd.Input{StorePackage: "com.example.store", Path: writeImage(t)}); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRun_storePackageFallsBackToEnv(t *testing.T) {
	...
	if _, err := imagecmd.Run(rc, imagecmd.Input{Package: "com.example.app", Path: writeImage(t)}); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRun_dryRun_noHTTPNoFileRead(t *testing.T) {
	rt := &testRoundTripper{}
	rc := newRC(t, rt)
	missing := filepath.Join(t.TempDir(), "not-rendered-yet.png")

	r, err := imagecmd.Run(rc, imagecmd.Input{StorePackage: "com.example.store", Package: "com.example.app", Path: missing, DryRun: true})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
	if len(rt.calls) != 0 {
...
func TestRun_missingStorePackage_exit2(t *testing.T) {
	...
	_, err := imagecmd.Run(rc, imagecmd.Input{Package: "com.example.app", Path: writeImage(t)})
	assertExit(t, err, 2)
...
func TestRun_missingFile_exit20(t *testing.T) {
	...
	_, err := imagecmd.Run(rc, imagecmd.Input{StorePackage: "com.example.store", Package: "com.example.app", Path: filepath.Join(t.TempDir(), "nope.png")})
	assertExit(t, err, 20)
	if rt.initURL != "" {
...
func TestRun_directory_exit20(t *testing.T) {
	...
	_, err := imagecmd.Run(rc, imagecmd.Input{StorePackage: "com.example.store", Package: "com.example.app", Path: t.TempDir()})
	assertExit(t, err, 20)
	if rt.initURL != "" {
...
func TestRun_403_namesEnrollment(t *testing.T) {
	...
	_, err := imagecmd.Run(rc, imagecmd.Input{StorePackage: "com.example.store", Package: "com.example.app", Path: writeImage(t)})
	assertExit(t, err, 11)
...
func TestRun_404_namesStorePackage(t *testing.T) {
	...
	_, err := imagecmd.Run(rc, imagecmd.Input{StorePackage: "com.unknown.store", Package: "com.example.app", Path: writeImage(t)})
	assertExit(t, err, 30)
	if !strings.Contains(err.Error(), "store-package") {
...
func TestNewCommand_helpDocumentsTheVerb(t *testing.T) {
	cmd := imagecmd.NewCommand(kernel.Boot{})
	if cmd.Use != "image <file>" {
		t.Errorf("Use = %q, want image <file>", cmd.Use)
	}
	if cmd.Short == "" {
	...
	// for, that nothing is shown until `appstore update`, the size/type limit,
	// and that no confirmation is required.
	for _, want := range []string{"imageid", "appiconid", "screenshotid", "15 mib", "image/*", "no --confirm", "nothing is shown"} {
