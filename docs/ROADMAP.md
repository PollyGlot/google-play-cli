...
(#245, lookup + refund gated, ADR-0031) sont désormais **livrées**, ainsi que le
gros bloc **monetization** (#51, subscriptions + IAP déclaratifs, ADR-0041,
clos 2026-07-28). Les **custom apps** (#242) sont **livrées** (PR #303), tout
comme le quick win read-only **generated APKs** (#299, trié depuis #294,
PR #305). La suite de la monetization sort du périmètre Google : la **sync
RevenueCat** ([#373](https://github.com/PollyGlot/google-play-cli/issues/373))
réconcilie le catalogue Play déclaratif avec un **troisième service, non-Google**
— première API tierce du CLI.
...
| ✅ | [#51](https://github.com/PollyGlot/google-play-cli/issues/51) | monetization | Subscriptions v2 + IAP one-shot & one-time products v2 — catalogue déclaratif | medium | **Livré** (clos 2026-07-28) — grillé le 2026-07-20 → 6 slices [#367](https://github.com/PollyGlot/google-play-cli/issues/367)–[#372](https://github.com/PollyGlot/google-play-cli/issues/372) ([ADR-0041](adr/0041-declarative-monetization-catalog.md)) : `subscriptions pull`/`apply` + `iap pull`/`apply` déclaratifs (stance **miroir** + gate `--confirm`, pull legacy-union, `--migrate` one-way, migration de prix abonnés gated) ; scope étendu le 2026-07-08 ([#293](https://github.com/PollyGlot/google-play-cli/issues/293) fusionné). La sync RevenueCat en a été **détachée** → [#373](https://github.com/PollyGlot/google-play-cli/issues/373) |
| prêt | [#373](https://github.com/PollyGlot/google-play-cli/issues/373) | monetization | Sync RevenueCat — réconcilier le catalogue Play avec products/entitlements/offerings/packages | medium | **Grillé le 2026-08-09** (débloqué par la clôture de #51) → 4 slices [#419](https://github.com/PollyGlot/google-play-cli/issues/419)–[#422](https://github.com/PollyGlot/google-play-cli/issues/422). Namespace `revenuecat` avec les verbes existants `pull`/`apply` (**pas** de verbe `sync`, ADR-0019) ; réconciliation **one-way Play → RevenueCat** (le modèle produit RC est un sous-ensemble strict de Play : l'inverse n'est pas exprimable) ; les **products RC sont dérivés** du catalogue #51 (`<productId>:<basePlanId>` par base plan), les **entitlements/offerings/packages sont déclarés** en fichiers ; deletes **et detaches** gated `--confirm` ; clé par `GPLAY_REVENUECAT_API_KEY` (env only, jamais un flag) ; REST v2 direct plutôt que le MCP RevenueCat. **Première API tierce (non-Google)** du CLI → ADR-0043 en slice 1 |
