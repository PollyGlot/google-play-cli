...
func ListAchievements(ctx context.Context, hc *http.Client, appID string, maxResults int, pageToken string) (AchievementListResponse, json.RawMessage, error) {
	req, err := newJSONReq(ctx, mAchList, opAchList, gamesApp(appID), map[string]string{"applicationId": appID}, listQuery(maxResults, pageToken), nil)
	if err != nil {
		return AchievementListResponse{}, nil, err
	}
	raw, err := do(hc, opAchList, gamesApp(appID), req)
	if err != nil {
		return AchievementListResponse{}, nil, err
	}
	var lr AchievementListResponse
	if err := json.Unmarshal(raw, &lr); err != nil {
		return AchievementListResponse{}, nil, &api.Error{Operation: opAchList, Resource: gamesApp(appID), Message: "decode response: " + err.Error(), Cause: err}
	}
	return lr, raw, nil
}
...
func GetAchievement(ctx context.Context, hc *http.Client, achievementID string) (AchievementConfiguration, json.RawMessage, error) {
	req, err := newJSONReq(ctx, mAchGet, opAchGet, achievement(achievementID), map[string]string{"achievementId": achievementID}, "", nil)
	if err != nil {
		return AchievementConfiguration{}, nil, err
	}
	return doAchievement(hc, opAchGet, achievement(achievementID), req)
}
...
func CreateAchievement(ctx context.Context, hc *http.Client, appID string, body []byte) (AchievementConfiguration, json.RawMessage, error) {
	req, err := newJSONReq(ctx, mAchInsert, opAchInsert, gamesApp(appID), map[string]string{"applicationId": appID}, "", body)
	if err != nil {
		return AchievementConfiguration{}, nil, err
	}
	return doAchievement(hc, opAchInsert, gamesApp(appID), req)
}
...
func UpdateAchievement(ctx context.Context, hc *http.Client, achievementID string, body []byte) (AchievementConfiguration, json.RawMessage, error) {
	req, err := newJSONReq(ctx, mAchUpdate, opAchUpdate, achievement(achievementID), map[string]string{"achievementId": achievementID}, "", body)
	if err != nil {
		return AchievementConfiguration{}, nil, err
	}
	return doAchievement(hc, opAchUpdate, achievement(achievementID), req)
}
...
func DeleteAchievement(ctx context.Context, hc *http.Client, achievementID string) error {
	req, err := newJSONReq(ctx, mAchDelete, opAchDelete, achievement(achievementID), map[string]string{"achievementId": achievementID}, "", nil)
	if err != nil {
		return err
	}
	_, err = do(hc, opAchDelete, achievement(achievementID), req)
	return err
}

// doAchievement executes req, parses the 2xx body as an AchievementConfiguration,
// and returns it with the verbatim raw body.
func doAchievement(hc *http.Client, op string, ref api.Resource, req *http.Request) (AchievementConfiguration, json.RawMessage, error) {
	...
	if err := json.Unmarshal(raw, &a); err != nil {
		return AchievementConfiguration{}, nil, &api.Error{Operation: op, Resource: ref, Message: "decode response: " + err.Error(), Cause: err}
	}
	return a, raw, nil
}
