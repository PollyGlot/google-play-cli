// Package set implements `gplay games leaderboards set <leaderboardId>`:
...
//
// set is a full PUT replace: for a partial edit fetch the current config
// (`view --output json`), edit it, and resend it with --file.
package set
...
func Run(rc *kernel.RunContext, in Input) (output.Renderable, error) {
	id, err := gamescmd.RequireID(in.ID, "missing leaderboard id: gplay games leaderboards set <leaderboardId>")
	if err != nil {
		return nil, err
	}
	...
	if in.DryRun {
		return gamescmd.LeaderboardWritePayload{Verb: "set leaderboard", Target: target, DryRun: true, Body: body, Cols: cols}, nil
	}
	...
	return gamescmd.LeaderboardWritePayload{Verb: "set leaderboard", Target: target, Row: gamescmd.BuildLeaderboardRow(l), Cols: cols, Raw: raw}, nil
}

// NewCommand returns the cobra command for `gplay games leaderboards set`.
func NewCommand(boot kernel.Boot) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "set <leaderboardId>",
		Short: "Set a leaderboard configuration (draft, full replace)",
		Long: `Set (replace) a leaderboard configuration by its ID. The write affects the editable
...
edit it, and resend it with --file. The field flags (--name, --score-order,
--score-min, --score-max; --locale, default en-US) send only what they name;
--file supplies a full body verbatim and is mutually exclusive with them.
...
shows the request body). GPLAY_READONLY refuses the live write (exit 4).`,
  ...
  gplay games leaderboards set CgkI4ZqN0pMUEAIQAg --file leaderboard.json --dry-run
  gplay games leaderboards set CgkI4ZqN0pMUEAIQAg --file leaderboard.json`,
