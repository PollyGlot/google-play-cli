...
import (
	...
	setcmd "github.com/PollyGlot/google-play-cli/commands/games/achievements/set"
...
func TestRun_happyPath_putsToResource(t *testing.T) {
	...
	_, err := setcmd.Run(rc, setcmd.Input{ID: "a7", Write: gamescmd.AchievementWrite{StepsToUnlock: 5, StepsSet: true}})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRun_explicitZeroFlag_reachesBody(t *testing.T) {
	...
	_, err := setcmd.Run(rc, setcmd.Input{ID: "a7", Write: gamescmd.AchievementWrite{PointValue: 0, PointValueSet: true}})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRun_missingID_exit2(t *testing.T) {
	...
	_, err := setcmd.Run(rc, setcmd.Input{Write: gamescmd.AchievementWrite{Name: "x"}})
