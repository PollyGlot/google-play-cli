...
| `contract.yml`: **Frozen surface needs a breaking marker or an ADR** | PR (also on title/body edit) | a PR that adds or removes a `frozen` line of `cmd/gplay/testdata/surface.golden` (regenerated by `make contract-update`) needs `!` in its title or an `ADR-NNNN` reference in its body (`scripts/contract-gate.sh`). Not required (yet). |
