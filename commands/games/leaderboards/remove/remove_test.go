...
import (
	...
	removecmd "github.com/PollyGlot/google-play-cli/commands/games/leaderboards/remove"
)
...
func TestRun_missingConfirm_exit3(t *testing.T) {
	...
	_, err := removecmd.Run(rc, removecmd.Input{ID: "l9"})
...
func TestRun_dryRun_doesNotCall(t *testing.T) {
	rc := gamescmdtest.NewRC(t, noCall(t))
	r, err := removecmd.Run(rc, removecmd.Input{ID: "l9", DryRun: true})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRun_happyPath_deletes(t *testing.T) {
	...
	if _, err := removecmd.Run(rc, removecmd.Input{ID: "l9", Confirm: true}); err != nil {
		t.Fatalf("Run: %v", err)
	}
