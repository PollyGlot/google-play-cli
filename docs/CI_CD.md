...
| `ci.yml`: **Docs sanity** | PR + push to `main` | verb gate (ADR-0019), em dash gate, shellcheck of the install scripts, the install script's fail-closed checksum test (offline), the gplay.sh Worker tests (`make worker-test`, offline: routing, headers, and `/install` serving the latest release tag, never `main`), required files. **Required check.** | none |
| `ci.yml`: **Fuzz smoke** | PR + push to `main` | every `Fuzz` target, run concurrently for a fixed number of executions (see [Fuzz smoke](#fuzz-smoke)). Not required. | none |
...
The `fuzz` job fuzzes every parser that takes untrusted bytes: API error
bodies, JWT claims, `Retry-After`, the reply-batch TSV, the reviews-report and
Data Safety CSVs, APK/AAB manifests, the credential redactor and the table cell
sanitiser. Their seed corpora already run as ordinary tests in `test`; this job
looks for crashers and broken properties in generated input. It is a smoke,
not a campaign, and it is not a required check.

- **Every target is listed.** The step fails when its target list and the
  `func Fuzz...` declarations in the tree differ, so a new target cannot end up
  replaying only its seeds.
- **All targets run at once**, one fuzz worker each, after one build of the
  shared instrumented dependencies (ten concurrent builds would each compile
  the standard library again). The job restores the `lint` job's living cache
  read-only and saves nothing.
- **Bounded by executions, not by the clock.** Each target stops after a fixed
  count (`-fuzztime=Nx`). The wall-clock bound it replaces flaked about once
  in a hundred runs with `context deadline exceeded`: the coordinator's
  deadline could expire while it was shutting down, even from a fresh corpus.
  A count sets no deadline. `timeout-minutes` caps the job instead.
- **No minimization** (`-fuzzminimizetime=0`). Minimizing each new input
  stalls a lone worker for seconds, and the smoke discards its corpus anyway.
  A crasher is still written to `testdata/fuzz/` in its package, and the step
  prints it so the log alone reproduces it.

Each count is about 35 s of work at the rate measured on the runner with all
ten targets sharing its 4 cores. When a target gets slower (a heavier parser,
a new property), lower its count rather than let the job grow: before this
layout the job fuzzed 3 targets in 127 s (median).
