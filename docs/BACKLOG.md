...
### IAP one-shot — `inappproducts` + `monetization.onetimeproducts` — ✅ **livré** (PRD [#51](https://github.com/PollyGlot/google-play-cli/issues/51), slices [#371](https://github.com/PollyGlot/google-play-cli/issues/371)/[#372](https://github.com/PollyGlot/google-play-cli/issues/372))
CRUD des produits in-app non-abonnement.
**Statut :** livré `[experimental]` — `gplay iap pull`/`apply` ([ADR-0041](adr/0041-declarative-monetization-catalog.md) §8) : `pull` lit **v2 ∪ legacy** et dédoublonne par product ID (un produit legacy non migré est invisible du list v2), `apply` n'écrit que le modèle **v2** ; la promotion legacy→v2 est un one-way door explicite (`--migrate`, exit 3 sans le flag).
**Pourquoi plus tard (rationnel d'origine) :** gros sous-domaine (CRUD complet + pricing par territoire). Sera un module à part entière post-MVP.

### Subscriptions v2 — `monetization.subscriptions` + `basePlans` + `offers` — ✅ **livré** (PRD [#51](https://github.com/PollyGlot/google-play-cli/issues/51), slices [#367](https://github.com/PollyGlot/google-play-cli/issues/367)–[#372](https://github.com/PollyGlot/google-play-cli/issues/372))
Abonnements, base plans, offers, pricing par territoire.
**Statut :** livré `[experimental]` — `subscriptions pull`/`apply` (et le sibling `iap pull`/`apply`) déclaratifs sur les 3 niveaux, moteur de réconciliation partagé ([ADR-0041](adr/0041-declarative-monetization-catalog.md)) : stance **miroir** (un produit live sans fichier = delete) + gate `--confirm`, state `ACTIVE`/`INACTIVE` réconcilié par `:activate`/`:deactivate`, `subscriptions prices convert`/`migrate` (migration d'abonnés gated, jamais déclenchée par un `apply`).
...
> La **sync RevenueCat** évoquée ci-dessus est **résolue par le PRD [#373](https://github.com/PollyGlot/google-play-cli/issues/373)** (voir la section suivante) : détachée de #51 dès le grilling du 2026-07-20, elle n'est plus une intention de backlog mais un PRD grillé et décomposé.

### Sync RevenueCat — 🔼 **PRD [#373](https://github.com/PollyGlot/google-play-cli/issues/373) grillé (2026-08-09) → slices [#419](https://github.com/PollyGlot/google-play-cli/issues/419)–[#422](https://github.com/PollyGlot/google-play-cli/issues/422)**
Réconcilier le [Monetization catalog](../CONTEXT.md) Play avec le catalogue RevenueCat (products / entitlements / offerings / packages) — miroir Android du skill iOS équivalent.
> **Grillé le 2026-08-09** (débloqué par la clôture de #51). Namespace `revenuecat` réutilisant les verbes déjà admis `pull`/`apply` — **pas** de verbe `sync` ([ADR-0019](adr/0019-canonical-verb-vocabulary.md)). Réconciliation **one-way Play → RevenueCat** : le modèle produit RC est un sous-ensemble strict de Play (ni prix, ni territoire, ni période de facturation), donc la direction inverse n'est pas exprimable, pas seulement repoussée. Les **products RC sont dérivés** du catalogue déclaratif (un product par **base plan**, `store_identifier` = `<productId>:<basePlanId>`) tandis que les **entitlements / offerings / packages sont déclarés** en fichiers sous `./monetization/revenuecat/`. Deletes **et detaches** gated `--confirm` (un detach révoque l'accès d'acheteurs existants). Clé lue de `GPLAY_REVENUECAT_API_KEY`, **jamais** d'un flag.
**Hors scope retenu :** direction RevenueCat → Play ; données runtime RC (customers, receipts, grants) — même logique d'exclusion qu'[ADR-0026](adr/0026-maximal-admin-api-coverage.md) applique à `purchases.*` ; price experiments, paywalls, RevenueCat Billing, virtual currencies ; création du projet/app RC ; stockage de la clé RC dans le keyring.
**Note de couverture :** RevenueCat est une **API tierce**, hors de la matrice [COVERAGE.md](COVERAGE.md) qui ne suit que les 4 admin APIs Google — c'est la première API non-Google du CLI, d'où un ADR dédié (ADR-0043, écrit en slice #419).
...
### Audit de cohérence read-only : ✅ **livré (`apps audit`, PRD [#449](https://github.com/PollyGlot/google-play-cli/issues/449), slices [#457](https://github.com/PollyGlot/google-play-cli/issues/457)/[#458](https://github.com/PollyGlot/google-play-cli/issues/458))**
Un compte dérive sans bruit : brouillons oubliés sur un track, locales présentes partout sauf sur une app, notes de version vides sur une release livrée, app qui n'a jamais rien publié en production. Rien ne fait remonter ça sans ouvrir la Play Console app par app.

**Décision de périmètre :** surface **composée**, zéro nouvelle méthode API. La découverte réutilise `apps.search` ([ADR-0039](adr/0039-accessible-apps-vs-local-registry.md)), chaque app est lue via l'Edit read-only existant (`edits.tracks.list` + `edits.listings.list`). Le rapport est un **document gplay** (pas un miroir verbatim : il compose plusieurs ressources, donc [ADR-0003](adr/0003-json-output-is-api-passthrough.md) n'a rien à refléter), d'où le classement `[experimental]` tant que le jeu de checks bouge. Nommé `apps audit` et non « account audit » : `CONTEXT.md` réserve **Account** au credential local et proscrit « account » non qualifié ; l'unité balayée est l'app.

**Nouveau code de sortie 70** (findings présents, DESIGN §9) : ni une erreur ni un succès muet, pour qu'un job CI distingue « a trouvé de la dérive » de « n'a pas pu regarder ».

**Hors périmètre (confirmé) :** toute correction automatique, les checks de conformité qui demanderaient une surface non couverte, et le cache (chaque run balaye en direct). Le check « production périmée » est livré comme `no-production-release` : la ressource Track ne porte aucune date sur une release, donc « périmé » n'est pas calculable par une lecture, « absent » l'est.
...
Faire graduer la formule du tap tiers `pollyglot/tap` vers `Homebrew/homebrew-core`, ce qui ferait afficher `gplay` en nom court au lieu du chemin `owner/repo/formule` — purement cosmétique, `brew install gplay` marche déjà en nom court via le tap.
