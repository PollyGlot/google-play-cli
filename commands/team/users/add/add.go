...
func (p Payload) renderMarkdown(w io.Writer) error {
	...
	rows := [][]string{
		{"User", p.Email},
		{"Action", p.verb()},
		{"Permissions", joinOrNone(p.Permissions)},
	}
	if len(p.Requires) > 0 {
...
func (p Payload) renderJSON(w io.Writer) error {
	if p.DryRun {
		return output.WriteJSON(w, dryRunView{
			DryRun:      true,
			Email:       p.Email,
			Permissions: nonNil(p.Permissions),
			Requires:    nonNil(p.Requires),
			Warnings:    p.Warnings,
		})
	}
	if len(p.Raw) == 0 {
...
func Run(rc *kernel.RunContext, in Input) (output.Renderable, error) {
	...
	raw, err := playteam.CreateUser(rc.Ctx, httpClient, developerID, playteam.User{
		Email:                       email,
		DeveloperAccountPermissions: enums,
	})
	if err != nil {
...
// emitWarnings prints any vocabulary steering warnings to stderr so they are
// visible regardless of --output format.
