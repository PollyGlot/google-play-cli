...
import (
	...
	submitcmd "github.com/PollyGlot/google-play-cli/commands/appstore/submit"
...
func TestRun_requestShape(t *testing.T) {
	...
	if _, err := submitcmd.Run(rc, submitcmd.Input{
...
func TestRun_unknownPolicyVariantSurvives(t *testing.T) {
	...
	if _, err := submitcmd.Run(newRC(t, rt, ""), submitcmd.Input{
...
func TestRun_readsStdin(t *testing.T) {
	for _, file := range []string{"", "-"} {
		t.Run("file="+file, func(t *testing.T) {
			...
			if _, err := submitcmd.Run(rc, submitcmd.Input{StorePackage: "s", Package: "com.example.app", File: file, Confirm: true}); err != nil {
				t.Fatalf("Run: %v", err)
			}
...
func TestRun_missingConfirm_exit3(t *testing.T) {
	rt := &testRoundTripper{}
	_, err := submitcmd.Run(newRC(t, rt, ""), submitcmd.Input{
...
func TestRun_dryRunNeedsNoConfirm(t *testing.T) {
	rt := &testRoundTripper{}
	got, err := submitcmd.Run(newRC(t, rt, ""), submitcmd.Input{
...
func TestRun_emptyBody_exit2(t *testing.T) {
	_, err := submitcmd.Run(newRC(t, &testRoundTripper{}, "   "), submitcmd.Input{
...
func TestRun_packageDefaultsToProjectPin(t *testing.T) {
	...
	if _, err := submitcmd.Run(rc, submitcmd.Input{StorePackage: "s", File: writeBody(t, fullBody), Confirm: true}); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRun_humanViews(t *testing.T) {
	got, err := submitcmd.Run(newRC(t, &testRoundTripper{}, ""), submitcmd.Input{
		StorePackage: "com.example.store", Package: "com.example.app", File: writeBody(t, fullBody), Confirm: true,
	})
	if err != nil {
		t.Fatalf("Run: %v", err)
...
func TestNewCommand_helpDocumentsTheVerb(t *testing.T) {
	cmd := submitcmd.NewCommand(kernel.Boot{})
	if cmd.Use != "submit" {
