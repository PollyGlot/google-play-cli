...
The Google service (`gamesConfiguration`) for configuring a game's Play Games Services resources — achievement and leaderboard configurations (CRUD on each: list/view/create/set/remove). An Admin API (developer configures their game), distinct from the Play Games Services *runtime* APIs the game itself calls (sign-in, score submission), which are out of scope. Addressed by the **Play Games application ID** (a numeric ID in its own space, not the Android package). The API writes the editable **draft**; the `published` copy is read-only and there is **no publish method** — publishing to players is Console-only ([ADR-0033](docs/adr/0033-games-services-configuration-draft-crud.md)). gplay surfaces it under the top-level `games` namespace (`games achievements` / `games leaderboards`, each with `list`/`view`/`create`/`set`/`remove`, the ADR-0019 verbs), addressed by `--application-id`; `remove` is the destructive tier (`--confirm`, exit 3), `create`/`set` accept localized field flags or a full `--file` body, and ships `[experimental]`.
...
### Artifact preflight
The **local** check gplay runs on an upload's file before any byte leaves the machine: the container's real structure (an AAB carries `BundleConfig.pb` and a protobuf manifest under its base module, an APK a binary-XML `AndroidManifest.xml` at its root, an [Expansion file](#expansion-file-obb) neither) must match the format the command resolved, and the **package name the artifact's manifest declares** must match the package being uploaded to. A mismatch fails offline at exit 20, naming expected and found, so no [Edit](#edit) is opened and no resumable upload session is reserved for a doomed artifact. Silent on success. `--skip-preflight` lifts the container and package checks (never the "missing or not a regular file" refusal, which predates the preflight), and anything gplay cannot classify degrades with a `NOTE:` on stderr rather than refusing: an unreadable manifest skips the package check, and a container past the member cap skips the container check too. The two manifest readers are hand-rolled and dependency-free ([ADR-0046](docs/adr/0046-artifact-preflight-hand-rolled-manifest-readers.md), in the spirit of [ADR-0007](docs/adr/0007-raw-http-not-google-go-sdk.md)). Applies on `releases upload`, `releases sharing upload`, `releases expansion-files upload`, `customapps create`, and `appstore apk upload`.

_Avoid_: calling it "validation" of the artifact (it checks identity and container, never signatures, `versionCode`s, or policy), or expecting it to replace Google's own server-side artifact rejection (exit 20 here means gplay refused locally; Google's refusal still arrives on its own terms).
...
### Generated APK
A signed APK that Play **generates from an uploaded AAB** for serving to devices — split, standalone, or universal APKs, plus asset-pack and recovery-module slices. Backed by the `generatedapks` resource of the [Android Publisher API](#android-publisher-api), keyed by package and `versionCode`, grouped in the response by **signing key** (each group carries a `certificateSha256Hash`). gplay surfaces it under `releases` (`releases generated list --version-code N` to enumerate, `releases generated download <downloadId> --version-code N --dest PATH` to fetch the bytes) — read-only and **Edit-free** (these endpoints are *not* under an [Edit](#edit), unlike a release upload). Use it after `releases upload` to verify the signing identity, sideload, or archive the exact artifacts Play serves ([ADR-0034](docs/adr/0034-generated-apks-binary-download-to-file.md)).

_Avoid_: treating it as an `edits`-namespace concept (there is no Edit), or confusing a **Generated APK** (Play's server-side output of bundle processing) with the AAB you uploaded or with a [Mapping](#mapping)/[Expansion file](#expansion-file-obb) (your own uploaded artifacts).
...
### Reconciliation plan
The action set `subscriptions apply` and `iap apply` compute between a [Monetization catalog](#monetization-catalog) and the live catalog: create/patch/delete of products and of their offers, plus the lifecycle transitions reconciling a declared `state:` (omitted means unmanaged) through the dedicated endpoints, never a patch: ACTIVE/INACTIVE on a base plan, a subscription offer or a purchase option, and ACTIVE/INACTIVE/CANCELLED on a [One-time product](#one-time-product) offer (CANCELLED is the pre-order `:cancel` verb). `--dry-run` prints the plan and stops; without it the plan executes, except that a plan containing any **delete**, or any offer **cancel** (irreversible: a cancelled offer never returns and its pending pre-orders are cancelled), refuses to run without `--confirm` (exit 3). `--output json` renders the plan as a gplay-owned shape (`[experimental]`), an explicit exception to pure API pass-through since no single API response describes a diff.
...
### Media tracking id
The identifier Google returns for one file uploaded against a [Hosted app](#hosted-app) — `apkId`, `imageId` or `fileId`, from `gplay appstore apk|image|policy upload` respectively. An upload is **inert**: it parks bytes on Google's side and hands back the id, distributing nothing. The id is the entire product of the call, and the only way to spend it is to cite it in a [Hosted app submission](#hosted-app-submission) — an APK id under `activeApks.activeApkSets[]` as `baseApkId` or a `splitApkId`, an image id as a listing's `appIconId` or one of its `screenshotId` entries, a policy document id as a policy response's `documentId`. Uploads reuse the shared resumable transfer path (PRD #355) and carry no confirmation gate, because inert-but-irreversible does not meet the [ADR-0043](docs/adr/0043-appstore-hosted-app-env-cascade-and-gate-placement.md) criterion.
...
### Update event
One entry of the Catalog Export's incremental-sync feed (`RecentUpdateEvent`, listed by `appstorecatalog.recentupdateevents.list`): `{playAppPackageName, eventTime, updateType}`, where the update type is **`MODIFICATION`** (the app changed — re-fetch its [Catalog app view](#catalog-app-view)) or **`DELETION`** (the app stopped being eligible for catalog inclusion or was removed from the Play Store — delist it). Listed by `gplay appstore catalog events list --since <t> --until <t>`: the time range is **required by the API**, RFC 3339, and half-open — start inclusive, end exclusive — and gplay validates it client-side so a malformed range costs no round trip. One page per invocation, `--page-size`/`--page-token`.
