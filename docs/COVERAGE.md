...
- **~98 shipped (~62%)** — essentially the entire *publish / first-release /
...
- **Monetization is started** — the declarative catalog
  ([ADR-0041](adr/0041-declarative-monetization-catalog.md)) shipped
  `subscriptions pull`/`apply` on the subscription top level (slice #367) and
  base-plan config + per-territory pricing incl. `prices convert` (slice
  #368); the reconciliation engine every remaining monetization slice
  (#369–#372) reuses is proven.
...
| `monetization.subscriptions` (+`basePlans`+`offers`) | 24 | 4 ✅ | `subscriptions pull`/`apply` — declarative catalog (`list`/`create`/`patch`/`delete`), slices [#367](https://github.com/PollyGlot/google-play-cli/issues/367)+[#368](https://github.com/PollyGlot/google-play-cli/issues/368) of [#51](https://github.com/PollyGlot/google-play-cli/issues/51) / [ADR-0041](adr/0041-declarative-monetization-catalog.md); **base-plan config incl. per-territory prices reconciled via the parent patch** (#368 — the sub-resource has no create/patch); `basePlans` state ops (`activate`/`deactivate`/`delete`/`batchUpdateStates`) ride [#369](https://github.com/PollyGlot/google-play-cli/issues/369), `migratePrices`/`batchMigratePrices` [#370](https://github.com/PollyGlot/google-play-cli/issues/370), `get`/`batchGet`/`batchUpdate`/`archive` + `offers` [#369](https://github.com/PollyGlot/google-play-cli/issues/369) 🟡 |
...
| `monetization.convertRegionPrices` | 1 | ✅ | `subscriptions prices convert` — pricing helper of the catalog, slice [#368](https://github.com/PollyGlot/google-play-cli/issues/368) ([ADR-0041](adr/0041-declarative-monetization-catalog.md) §8) |
